...
function leqPredicates(ps1, ps2) {
  return (
    ps2.length <= ps1.length &&
    ps2.every(function(p) {
...
function leq(A, B) {
  ...
  // Let A be a refinement then A <= B if decompose(A) <= decompose(B)
    ...
    var dB = decompose(B);
    return (
      leqPredicates(dA.predicates, dB.predicates) && recurse(dA.unrefinedType, dB.unrefinedType)
    );
  }
  ...
  // Let A be a list then A <= B if one of the following holds:
    ...
    return (
      kindB === 'dict' &&
      recurse(A.meta.domain, B.meta.domain) &&
      recurse(A.meta.codomain, B.meta.codomain)
    );
  }
  ...
  // Let A be a function then then A <= B if one of the following holds:
    ...
    return (
      kindB === 'func' &&
      recurse(A.meta.codomain, B.meta.codomain) &&
      leqList(A.meta.domain, B.meta.domain)
    );
  }
  ...
  // Let A be an interface then A <= B if one of the following holds:
    ...
    if (kindB === 'interface') {
      ...
      return (
        compatible && A.meta.strict === true && keysB.length === Object.keys(A.meta.props).length
      );
    }
  }
...
function isSubsetOf(A, B) {
  if (process.env.NODE_ENV !== 'production') {
    assert(isType(A), function() {
      return `Invalid argument subset ${assert.stringify(
        A,
      )} supplied to isSubsetOf(subset, superset) (expected a type)`;
    });
    assert(isType(B), function() {
      return `Invalid argument superset ${assert.stringify(
        B,
      )} supplied to isSubsetOf(subset, superset) (expected a type)`;
    });
  }
