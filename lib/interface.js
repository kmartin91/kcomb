...
function inter(props, options) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    assert(isTypeName(name), function() {
      return `Invalid argument name ${assert.stringify(
        name,
      )} supplied to interface(props, [options]) combinator (expected a string)`;
    });
  ...
  const displayName = name || getDefaultInterfaceName(props);
  const identity = Object.keys(props)
    .map(function(prop) {
      return props[prop];
    })
    .every(isIdentity);
    ...
    for (const prop in props) {
      ...
      const instance = create(
        ...
        process.env.NODE_ENV !== 'production'
          ? path.concat(`${prop}: ${getTypeName(expected)}`)
          : null,
      );
