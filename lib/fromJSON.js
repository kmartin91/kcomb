...
function fromJSON(value, type, path) {
  if (process.env.NODE_ENV !== 'production') {
    assert(isFunction(type), function() {
      return `Invalid argument type ${assert.stringify(
        type,
      )} supplied to fromJSON(value, type) (expected a type)`;
    });
  ...
  switch (kind) {
    ...
    case 'subtype': // the kind of a refinement is 'subtype' (for legacy reasons)
      ...
      if (process.env.NODE_ENV !== 'production') {
        assert(type.meta.predicate(ret), function() {
          return `Invalid argument value ${assert.stringify(
            value,
          )} supplied to fromJSON(value, type) (expected a valid ${getTypeName(type)})`;
        });
      }
      return ret;
    ...
    case 'struct':
      if (process.env.NODE_ENV !== 'production') {
        assert(isObject(value), function() {
          return `Invalid argument value ${assert.stringify(
            value,
          )} supplied to fromJSON(value, type) (expected an object for type ${getTypeName(type)})`;
        });
      }
      var { props } = type.meta;
      var { defaultProps } = type.meta;
      ret = {};
      for (k in props) {
        let actual = value[k];
        if (actual === undefined) {
        ...
        if (props.hasOwnProperty(k)) {
          ret[k] = fromJSON(
            ...
            process.env.NODE_ENV !== 'production'
              ? path.concat(`${k}: ${getTypeName(props[k])}`)
              : null,
          );
        }
      }
    ...
    case 'interface':
      if (process.env.NODE_ENV !== 'production') {
        assert(isObject(value), function() {
          return `Invalid argument value ${assert.stringify(
            value,
          )} supplied to fromJSON(value, type) (expected an object)`;
        });
      }
      ...
      for (k in interProps) {
        if (interProps.hasOwnProperty(k)) {
          ret[k] = fromJSON(
            ...
            interProps[k],
            process.env.NODE_ENV !== 'production'
              ? path.concat(`${k}: ${getTypeName(interProps[k])}`)
              : null,
          );
        }
      }
    ...
    case 'list':
      if (process.env.NODE_ENV !== 'production') {
        assert(isArray(value), function() {
          return `Invalid argument value ${assert.stringify(
            value,
          )} supplied to fromJSON(value, type) (expected an array for type ${getTypeName(type)})`;
        });
      }
      ...
      return value.map(function(element, i) {
        return fromJSON(
          element,
          elementType,
          process.env.NODE_ENV !== 'production' ? path.concat(`${i}: ${elementTypeName}`) : null,
        );
      });
    ...
    case 'union':
      ...
      if (process.env.NODE_ENV !== 'production') {
        assert(isFunction(actualType), function() {
          return `Invalid argument value ${assert.stringify(
            value,
          )} supplied to fromJSON(value, type) (no constructor returned by dispatch of union ${getTypeName(type)})`;
        });
      }
      ...
      if (process.env.NODE_ENV !== 'production') {
        assert(isArray(value) && value.length === types.length, function() {
          return `Invalid value ${assert.stringify(
            value,
          )} supplied to fromJSON(value, type) (expected an array of length ${types.length} for type ${getTypeName(type)})`;
        });
      }
      return value.map(function(element, i) {
        return fromJSON(
          ...
          types[i],
          process.env.NODE_ENV !== 'production'
            ? path.concat(`${i}: ${getTypeName(types[i])}`)
            : null,
        );
      });
    ...
    case 'dict':
      ...
      var { domain } = type.meta;
      var { codomain } = type.meta;
      ...
      for (k in value) {
        if (value.hasOwnProperty(k)) {
          ret[
            domain(k, process.env.NODE_ENV !== 'production' ? path.concat(domainName) : null)
          ] = fromJSON(
            value[k],
            codomain,
            process.env.NODE_ENV !== 'production' ? path.concat(`${k}: ${codomainName}`) : null,
          );
        }
      }
      return ret;
    ...
    case 'intersection':
      var values = type.meta.types.map(function(type, i) {
        return fromJSON(
          value,
          type,
          process.env.NODE_ENV !== 'production' ? path.concat(`${i}: ${getTypeName(type)}`) : null,
        );
      });
      return type(isObject(values[0]) ? assignMany(values) : value, path);
